...
void handleRoot() {
  String credentials = "{\"ssid\":\"" + WiFi.SSID() + "\",\"password\":\"" + WiFi.psk() + "\"}";
  server.send(200, "application/json", credentials);
}
...
void setup() {
  ...
  // Configurar SoftAP
  WiFi.softAP("ESP-Maestro-Config", "config123");
  ...
  Serial.print("ESP Maestro conectado a la red: ");
  Serial.println(WiFi.SSID());

  // Guardar credenciales en EEPROM
  EEPROM.begin(97); // Aumentar el tamaño de la EEPROM

  String ssid = WiFi.SSID();
  String password = WiFi.psk();

  for (int i = 0; i < ssid.length(); i++) {
    EEPROM.write(EEPROM_SSID_ADDR + i, ssid.charAt(i));
  }
  EEPROM.write(EEPROM_SSID_ADDR + ssid.length(), '\0');

  for (int i = 0; i < password.length(); i++) {
    EEPROM.write(EEPROM_PASSWORD_ADDR + i, password.charAt(i));
  }
  EEPROM.write(EEPROM_PASSWORD_ADDR + password.length(), '\0');

  // Activar la bandera
  EEPROM.write(EEPROM_FLAG_ADDR, 1);
  Serial.println("Bandera activada en EEPROM"); // Añadir esta línea
  delay(100); // Añadir un pequeño retraso
  EEPROM.commit();
  delay(100); // Añadir un pequeño retraso
}
